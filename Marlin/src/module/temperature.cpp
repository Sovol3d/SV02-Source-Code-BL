  ...
  float Temperature::get_pid_output_hotend(const uint8_t E_NAME) {
    const uint8_t ee = HOTEND_INDEX;
        ...
        float pid_output;
			...
			if(!room_temperature)
			{
			  room_temperature = temp_hotend[0].celsius;
			}

			if((pid_error <= 0) && (PrinterStatusKey[1] == 1))
			{
			  if(LanguageRecbuf != 0)
			  {
				// 7 for Heating Done
			...
			else if((PrinterStatusKey[1] == 2) && ((!temp_hotend[ee].target && pid_error >= -(room_temperature + 5)) ||(temp_hotend[ee].target && pid_error >= 0)))
			{
			  if(LanguageRecbuf != 0)
			  {
				// 9 for Cooling Done
...
void Temperature::tick() {
  ...
  void Temperature::print_heater_states(const uint8_t target_extruder
    ...
    bool Temperature::wait_for_hotend(const uint8_t target_extruder, const bool no_wait_for_cooling/*=true*/
        ...
        #define TEMP_REACHC (wants_to_cool ? isCoolingHotend(target_extruder) : isHeatingHotend(target_extruder))
	  #else
