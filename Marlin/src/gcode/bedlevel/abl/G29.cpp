...
G29_TYPE GcodeSuite::G29() {
  if((G29_status == 1) && (PrinterStatusKey[0] == 1) && !AutoLevelStatus)
  {
    return;
  }
  G29_status = 1;
      ...
      for (PR_OUTER_VAR = 0,showcount = 0; PR_OUTER_VAR < PR_OUTER_END && !isnan(measured_z); PR_OUTER_VAR++) {
        ...
        for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc) {
          ...
          if (isnan(measured_z)) {
            ...
            rtscheck.RTS_SndData(z_values[meshCount.x][meshCount.y] * 10000, AutolevelVal + (showcount-1)*2);
            #endif
		  #endif
  ...
  if(PrintModeTime) PrintModeTime = print_job_timer.duration();
